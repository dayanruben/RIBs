...
RIBs is the cross-platform architecture behind many mobile apps at Uber. The name RIBs is the short of Router, Interactor, Builder, which are core components of this architecture. This framework is designed for mobile apps with a large number of engineers and nested states.
...
To read about the backstory on why we created RIBs, see [this blog post](https://eng.uber.com/new-rider-app/) we wrote when releasing RIBs in production the first time.
